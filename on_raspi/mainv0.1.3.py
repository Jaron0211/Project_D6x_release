...
def send_VISION_POSITION_ESTIMATE(x,y,z,roll = 0,pitch = 0,yaw = 0):
    msg = pix.message_factory.vision_position_estimate_encode(
...
def task_manager(frame):
    ...
    log_msg += "\npitch: %f roll: %f  yaw: %f thrust: %f " % (round(pitch_angle,1), round(roll_angle,2), round(yaw_rate,2), round(thrust,2))
...
while(1):
    ...
    if(mission_start):
        ...
        log_msg += '\nControlMode: Manual'
    send_VISION_POSITION_ESTIMATE(y=-XV / 100, x=-YV / 100, z=ZV / 100, yaw=AV / 180 * 3.14159)
    ##global log
